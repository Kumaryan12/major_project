...
def prepare(config: dict) -> list[dict[str, str]]:
    data = config["data"]
    return prepare_database(data["database_url"], Path(data["raw_dir"]))
...
def featurize(config: dict) -> Path:
    ...
    for record_index, item in enumerate(manifest, 1):
        ...
        for beat_index, (beat, peak) in enumerate(zip(beats, valid_peaks, strict=True)):
            vectors.append(extract_features(
                beat,
            ...
            rows.append({**item, "beat_index": beat_index, "r_peak": int(peak)})
